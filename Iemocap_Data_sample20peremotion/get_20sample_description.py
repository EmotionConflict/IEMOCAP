...
au_des_path = os.path.join(base_path,"output_au")
...
os.makedirs(os.path.join(base_path,"sample20peremotion","video"),exist_ok=True)
...
df = pd.read_csv("/Users/summerghorbani/Documents/MIT_projects/How2AI/IEMOCAP_full_release/test_video/sample20peremotion/sample_20_per_emotion.csv")
# df_conf = df
# # [df["Emotion"] == "xxx"]
# rows_all=[]

# df_all = pd.DataFrame(columns=["File_Name","Emotion"])
# for _,row in df_conf.iterrows():
#     file_name = row["File_Name"]
#     video_path = os.path.join(video_des_path,f"{file_name}.csv")
...
#     if os.path.exists(video_path) and os.path.exists(audio_path) and os.path.exists(text_path) and os.path.exists(au_path):
#         print(f"Copying {file_name} descriptions")
#         rows_all.append(row)
#         cpy_video_path = os.path.join(base_path,"sample20peremotion","video",f"{file_name}.csv")
#         cpy_audio_path = os.path.join(base_path,"sample20peremotion","audio",f"{file_name}.txt")
#         cpy_text_path = os.path.join(base_path,"sample20peremotion","text",f"{file_name}.txt")
#         cpy_au_path = os.path.join(base_path,"sample20peremotion","au",f"{file_name}.csv")
#         #copy video description
#         shutil.copy(video_path,cpy_video_path)
#         #copy audio description
#         shutil.copy(audio_path, cpy_audio_path)
#         #copy text description
#         shutil.copy(text_path, cpy_text_path)
#         #copy au description
#         shutil.copy(au_path ,cpy_au_path)

# df_all = pd.DataFrame(rows_all,columns=["File_Name","Emotion"])

# df_all.to_csv(os.path.join(base_path,"sample20peremotion","available_samples_20_per_emotion.csv"),index=False)
#         #copy audio description
#         #copy video description
...
for _, row in df.iterrows():
    ...
    if all(os.path.exists(p) for p in [video_path, audio_path, text_path, au_path]):
        ...
        shutil.copy(video_path, os.path.join(base_path, "sample20peremotion", "video", f"{file_name}.csv"))
